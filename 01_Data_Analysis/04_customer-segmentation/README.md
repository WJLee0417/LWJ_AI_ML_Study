...
정책 매트릭스는 [campaign-priority-matrix.md](results/campaign-priority-matrix.md)에서 확인할 수 있다. 이는 **운영 설계 예시**이며, Olist RFM 데이터와 Telco 이탈 데이터가 서로 다른 고객 집합이므로 실제 고객 단위 결합 결과가 아니다.
