...
def parse_args() -> argparse.Namespace:
    defaults = {"model": "resnet-finetune", "run_name": None, "model_version": None, "epochs": 20, "patience": 5, "batch_size": 32, "learning_rate": 1e-3, "augmentation": "on", "class_weighting": "balanced", "seed": 42, "num_workers": 0, "processed_dir": "data/processed", "models_dir": "models", "results_dir": "results", "assets_dir": "assets", "device": "auto"}
    parser = parser_with_config("Train a waste-classification transfer-learning experiment.", defaults)
    parser.add_argument("--model", choices=["scratch", "resnet-feature", "resnet-finetune"], default=argparse.SUPPRESS)
    parser.add_argument("--run-name", default=argparse.SUPPRESS)
    parser.add_argument("--model-version", default=argparse.SUPPRESS)
    ...
    parser.add_argument("--augmentation", choices=["on", "off"], default=argparse.SUPPRESS)
    parser.add_argument("--class-weighting", choices=["balanced", "none"], default=argparse.SUPPRESS)
    parser.add_argument("--no-pretrained", action="store_true")
    parser.add_argument("--seed", type=int, default=argparse.SUPPRESS)
    parser.add_argument("--num-workers", type=int, default=argparse.SUPPRESS)
    parser.add_argument("--processed-dir", default=argparse.SUPPRESS)
    parser.add_argument("--models-dir", default=argparse.SUPPRESS)
    parser.add_argument("--results-dir", default=argparse.SUPPRESS)
    parser.add_argument("--assets-dir", default=argparse.SUPPRESS)
...
def main() -> None:
    ...
    model_version = args.model_version or f"{args.model}-v1"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if validation_metrics["macro_f1"] > best_f1:
            ...
            torch.save({"architecture": args.model, "model_version": model_version, "class_names": class_names, "image_size": IMAGE_SIZE, "normalization": "imagenet", "augmentation": args.augmentation, "class_weighting": args.class_weighting, "epoch": epoch, "validation_macro_f1": best_f1, "state_dict": model.state_dict()}, checkpoint_path)
        else:
            stale_epochs += 1
    ...
    summary = {"experiment": run_name, "model": args.model, "model_version": model_version, "augmentation": args.augmentation, "class_weighting": args.class_weighting, "best_validation_macro_f1": best_f1, "training_time_seconds": elapsed, "trainable_parameters": count_parameters(model), "total_parameters": count_parameters(model, trainable_only=False), "checkpoint_size_bytes": checkpoint_path.stat().st_size}
